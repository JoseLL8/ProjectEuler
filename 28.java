...
class Problem28 {
	public static void main(String[] args) { //this problem is solved supposing that the diagonals start at 1, which only really makes sense for uneven dimension, which are both the example (5) and the question (1001) provided by the problem
		int limit = (int)Math.pow(Integer.parseInt(args[0]),2); //spiarls with cuadratic dimensions obviously have the same number of elemens as quadratic matrices, that is, n²
		int i = 1;
		int step = 2;
		int prev = -1;
		int result = 0;
