...
class Problem38 {
	public static void main(String[] args) {
		...
		long aux;
		String out;
		for (int c=1; c<(int)Math.pow(10,digits/2); c++) { //n>1 according to problem description, so the max value will probably be at best roughly 10^digits/2
			out ="";
			p = 0;
			while (out.length()<digits) {
				p++;
				out += String.valueOf(c*p);
			}
			if (out.length()==digits && p>1) if (checkPandigit(out, digits)) {
				System.out.println("c="+c+"; p="+p);
				aux = Long.parseLong(out);
				if (aux>max) max=aux;
			}
		}
