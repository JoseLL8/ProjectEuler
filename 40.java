...
class Problem40 {
	public static void main(String[] args) {
		...
		//System.out.println(champernowne(target));
		System.out.println(champernowne(1)*champernowne(10)*champernowne(100)*champernowne(1000)*champernowne(10000)*champernowne(100000)*champernowne(1000000));
	}

	public static int champernowne(int target) {
		int step = 1;
		int count = 0;
		for (int i=1; count<target; i++) {
			if (i>=(int)Math.pow(10,step)) {step++;}
			count += step;
			if (count>=target) {return String.valueOf(i).charAt(String.valueOf(i).length()-1-count+target) - '0';}
		}
		return 0; //the compiler forces me to put this in
	}
